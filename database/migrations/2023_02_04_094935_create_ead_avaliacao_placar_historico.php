...
class CreateEadAvaliacaoPlacarHistorico extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('ead_avaliacao_placar_historico', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreign('ead_avaliacao_id','fk_ava_placar_histo_ava')->references('id')->on('ead_avaliacao');
            $table->foreign('user_id','fk_ava_placar_histo_user')->references('id')->on('users');
        });
    }
    ...
    {
        Schema::dropIfExists('ead_avaliacao_placar_historico');
    }
}
