...
<section class="section section-interna">
	<div class="container">
						...
						<div class="filter">
							<form id="formPesquisa" action="" method="get">
								...
								<div class="form-row">
                                    ...
                                    <div class="form-group col-md-3 ">
                                        <div class="input-group">
                                            <select name="exibicao" id="exibicao" class="form-control multiple11" >
                                    ...
                                    <div class="form-group col-md-9 ">
                                        <div class="col text-right">
                                            ...
                                            <a href="{{url('/gestao/avaliacao/minhas_questoes')}}" class="btn btn-danger fs-13" title="Limpar Filtro"><i class="fas fa-undo-alt"></i> Limpar todos os filtros</a>
                                        </div>
                                    </div>
								</div>
		...
		<div class="row @if(auth()->user()->permissao == 'Z') pb-4 @endif " >
			<div class="col-md-6">
				<h4>Questões
            ...
            <div class="col-md-6 text-right">
                <span class="mr-5 "><a href="{{url('/gestao/avaliacao')}}" class="btn btn-warning text-white"><i class="fas fa-list"></i> Minhas avaliações</a></span>
				<span><a href="{{url('/gestao/avaliacao/minhas_questoes/nova')}}" class="btn btn-success"><i class="fas fa-plus"></i> Criar Nova Questão</a></span>
			</div>
		</div>
                ...
                <div class="col-12">
                    <h4>
                        <ul class="nav nav-tabs">
                            <li class="nav-item">
                                <a class="nav-link @if(Request::input('biblioteca') == '' ) active @endif" href="javascript:$('#biblioteca').val('');$('#formPesquisa').submit()">Minhas questões</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link @if(Request::input('biblioteca') == 1 ) active @endif" href="javascript:$('#biblioteca').val(1);$('#formPesquisa').submit()">Questões da biblioteca</a>
                            </li>
                        </ul>
                    </h4>
                </div>
                        ...
                        <table class="table table-striped">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col"></th>
                                    @if( count($dados)>0 && ((auth()->user()->permissao == 'Z' && $dados[0]->instituicao_id == 1) ) )
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{substr($pergunta,0,60)}}@if(strlen($pergunta)>60)... @endif
                                        <br>
                                        <div class="mt-2 fs-11" style="font-size: 11px; color: #E65A24"><i class="fas fa-layer-group"></i> {{@$d->cicloEtapa->ciclo}} / {{@$d->cicloEtapa->ciclo_etapa}} &nbsp;&nbsp;&nbsp; <i class="fas fa-book"></i> {{$d->disciplina->titulo}} &nbsp;&nbsp;&nbsp; <i class="fas fa-ruler-vertical"></i> {{$dificuldade[$d->dificuldade]}} &nbsp;&nbsp;&nbsp; @if(isset($d->usuario)&&$d->usuario->name) <i class="fas fa-user"></i> {{$d->usuario->name}} @endif</div>
                                    </td>
                                    ...
                                    @if(auth()->user()->permissao == 'Z' && $d->instituicao_id == 1)
                                        @if($d->publicado == 0)
                                        <td class="text-center">
                                            <div class="custom-control custom-switch">
                                                <input onchange="mudaRevisao({{$d->id}},this,'cadastro')" type="checkbox" @if($d->cadastro==1) checked @endif  class="custom-control-input" id="statusCadastro{{$d->id}}" value="1">
                                                <label class="custom-control-label pt-1" for="statusCadastro{{$d->id}}"></label>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="custom-control custom-switch" >
                                                <input onchange="mudaRevisao({{$d->id}},this,'revisado')" type="checkbox" @if($d->revisado==1) checked @endif  class="custom-control-input" id="statusRevisado{{$d->id}}" value="1">
                                                <label class="custom-control-label pt-1" for="statusRevisado{{$d->id}}"></label>
                                            </div>
                                        </td>
                                        @else
                                            <td></td><td></td>
                                    ...
                                    <td class="text-right text-nowrap">
                                        @if ( (auth()->user()->permissao == 'Z' && $d->intituicao_id==1) || (auth()->user()->id == $d->user_id && auth()->user()->escola_id == $d->escola_id) )
                                            <span><a href="{{url('/gestao/avaliacao/minhas_questoes/publicar/'.$d->id)}}"  class="btn btn-secondary btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="@if($d->publicado==1) Despublicar @else Publicar @endif"><i class="fas fa-bullhorn"></i> </a></span>
                                        @endif
                                        <span>
                                            <a href="{{url('/gestao/avaliacao/minhas_questoes/duplicar/'.$d->id)}}" class="btn btn-secondary btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="@if(Request::input('biblioteca') == 1 ) Adicionar as minhas questões @else Duplicar @endif">
                                                @if(Request::input('biblioteca') == 1 )
                                        ...
                                        @if($d->user_id == auth()->user()->id || (auth()->user()->permissao == 'Z' && $d->instituicao_id == 1) )
                                            <span><a href="{{url('/gestao/avaliacao/minhas_questoes/editar/'.$d->id)}}" class="btn btn-secondary btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar Questão"><i class="fas fa-pencil-alt"></i></a></span>
...
<script type="text/javascript">
    ...
    var selFonte= new vanillaSelectBox(".multiple11", {
        "maxHeight": 250,
	...
	function excluir()
    {
        window.location.href = '{{url("/gestao/avaliacao/minhas_questoes/excluir")}}/'+idExcluir;
    }
    ...
    function mudaRevisao(idQuestao,elemento,tipo)
    {
        ...
        $.ajax({
                url: '{{url('/gestao/avaliacao/minhas_questoes/mudaStatus')}}',
                type: 'post',
                dataType: 'json',
                data: dados,
                error: function(data) {
