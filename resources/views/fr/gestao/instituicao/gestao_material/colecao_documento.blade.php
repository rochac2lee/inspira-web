...
@section('content')
    ...
    <section class="section section-interna">
        <div class="container">
            ...
            <h2 class="title-page">
                ...
                Coleções de Documentos Oficiais Permitidos<br><small>{{$instituicao->titulo}}</small>
            </h2>
                <form action="{{url('/gestao/instituicao/material/addColecaoDocumento/')}}" method="post">
                    <div class="form-row">
                        ...
                        <div class="col-md-6">
                            <select name="colecao[]" class="selectColecao" style="border: 1px solid #ffb100; border-radius: 0.4rem;" multiple>
                                @foreach($colecaoParaAdd as $add)
                                    <option value="{{$add->id}}">{{$add->nome}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                    ...
                    @foreach($dados as $d)
                        <div class="col-md-6 grid-item">
                            <div class="card text-center">
                                <div class="card-body">
                                    <div class="img">
                                        <img class="img-fluid" src="{{ config('app.cdn').'/storage/colecaodocumento/'.$d->img}}" />
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <strong class="title h6 font-weight-bold d-block">{{$d->nome}}</strong>

                                        <p id="badgeTodos" @if($d->todos != 1) style="display: none" @endif><span class="badge badge-success" > Todos Etapa / Ano</span></p>
                                        <p id="badgeParcial" @if($d->todos == 1) style="display: none" @endif><span class="badge badge-danger" >Parcial Etapa / Ano</span></p>
                                    ...
                                    <a href="javascript:void(0)" onclick="$('#etapa{{$d->id}}').toggle()" alt="Gerenciar Etapa / Ano" title="Gerenciar Etapa / Ano" class="btn btn-secondary">
                                        <i class="fas fa-server"></i> Gerenciar Etapa / Ano
                                    </a>
                                    ...
                                    <div id="etapa{{$d->id}}" style="display: none;" class="text-left">
                                        <form action="{{url('/gestao/instituicao/material/permissaoColecaoDocumento')}}" method="post" >
                                            <br><br>
                                            @csrf
                                            <input type="hidden" name="instituicao_id" value="{{$instituicao->id}}">
                                            ...
                                            <div class="form-group">
                                                @foreach($d->cicloEtapaColecao as $c)
                                                    ...
                                                    <div class="form-check">
                                                        <input class="form-check-input rounded" {{$checked}} name="cicloetapa[]" type="checkbox" value="{{$c->cicloetapa_id}}" id="ck{{$c->cicloetapa_id}}">
                                                        <label class="form-check-label" for="ck{{$c->cicloetapa_id}}">
                                            ...
                                            <button id="btnForm" type="submit" class="btn btn-success">Salvar Etapa / Ano</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="col">
                    <div class="card text-center">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">Essa instituição ainda não possui coleções de documentos adicionadas.</p>
                        </div>
    ...
    <script>
        ...
        function excluir()
        {
            window.location.href = '{{url('/gestao/instituicao')}}/' + vinstituicaoId + '/material/removerColecaoDocumento/' + vcolecaoId;
        }
