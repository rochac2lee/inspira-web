<form action="{{url('/gestao/agenda/registros/buscar')}}" method="get" autocomplete="off">
    ...
    <div class="form-row">
        ...
        <div class="col-4">
            ...
            <select class="selProfessor " id="selProfessor" name="professor_id" style="border: 1px solid {{ $errors->has('turma_id') ? '#DC3545' : '#ffb100' }} ; border-radius: 0.4rem;" onchange="mudouProfessor()">
                <option value="">Selecione</option>
                @foreach($professores as $p)
                    <option @if(@$professor_id == $p->id) selected @endif value="{{$p->id}}">{{$p->nome}}</option>
                @endforeach
            </select>
...
<script>
    ...
    $(document).ready(function(){
        ...
        var selProfessor = new SlimSelect({
            select: '.selProfessor',
            placeholder: 'Buscar',
