    ...
    <section class="section section-interna" style="padding-top: 50px">
        <div class="container">
            ...
            <div class="row mb-3 border-top pt-4">
                <div class="col-md-12">
                    <div class="filter">
                        <form id="formPesquisa" class="form-inline d-flex justify-content-end">
                            <input type="hidden" name="enviados" value="{{Request::input('enviados')}}" id="enviados">
            ...
            <div class="row pb-4">
                <div class="col-md-4">
                    ...
                    <div class="col-md-8 text-right">
                        <a href="{{url('gestao/agenda/documentos/novo')}}" class="btn btn-success" >
            ...
            <div class="row">
                <h4 style="margin-bottom: 18px;">
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <a class="nav-link @if(Request::input('enviados') == '' ) active @endif" href="javascript:$('#enviados').val('');$('#formPesquisa').submit();">Documentos solicitados</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link @if(Request::input('enviados') == 1 ) active @endif" href="javascript:$('#enviados').val(1);$('#formPesquisa').submit();">Documentos enviados</a>
                        </li>
                    </ul>
                </h4>
            </div>
                                ...
                                @foreach($dados as $d)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if($d->arquivo != '')
                                                <br><a href="#"><span class="badge badge-info" style="font-weight: normal"><i class="far fa-file"></i> {{$d->nome_arquivo_original}} </span></a>
                                            @else
                                                <br><span class="badge @if(count($d->recebidos)==0) badge-secondary @else badge-primary @endif" style="font-weight: normal"><i class="fas fa-upload"></i> {{count($d->recebidos)}} recebidos</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($d->publicado == 1)
                                        ...
                                        <td class="text-right text-nowrap">
                                            ...
                                            @if( ( auth()->user()->permissao == 'I' ||  (auth()->user()->permissao == 'P' && $d->user_id == auth()->user()->id) || (auth()->user()->permissao == 'C' && ($d->user_id == auth()->user()->id || $d->permissao_usuario == 'P') ) )  )
                                                ...
                                                <span><a href="{{url('/gestao/agenda/documentos/editar/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pencil-alt"></i></a></span>
                                                ...
                                                @elseif($d->arquivo == '')
                                                    <span><a href="{{url('/gestao/agenda/documentos/recebidos/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Recebidos"><i class="fas fa-upload"></i></a></span>
                                                @endif
                                            @endif
                                        </td>
                                    </tr>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Exclusão de Documento</h5>
                </div>
...
<script>
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/documentos/excluir")}}/'+idExcluir;
    }
    ...
    function publicar()
    {
        window.location.href = '{{url("/gestao/agenda/documentos/publicar")}}/'+idPublicar;
    }

    function modalVisualizar(id)
    {
        $.ajax({
            url: '{{url('gestao/agenda/documentos/exibir')}}/'+id,
            type: 'post',
            dataType: 'json',
            data: {
                _token: '{{csrf_token()}}'
