            ...
            <div class="row pb-4">
                <div class="col-md-4">
                    ...
                    <div class="col-md-8 text-right">
                        <a href="{{url('gestao/agenda/enquetes/novo')}}" class="btn btn-success" >
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right text-nowrap">
                                            ...
                                            @if( ( auth()->user()->permissao == 'I' ||  (auth()->user()->permissao == 'P' && $d->user_id == auth()->user()->id) || (auth()->user()->permissao == 'C' && ($d->user_id == auth()->user()->id || $d->permissao_usuario == 'P') ) )  )
                                                @if($d->publicado == 0 )
                                                <span><button class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="@if($d->publicado==1) Despublicar @else Publicar @endif" onclick="modalPublicar({{$d->id}}, '{{$d->pergunta}}')"><i class="fas fa-bullhorn"></i></button></span>
                                                <span><a href="{{url('/gestao/agenda/enquetes/editar/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pencil-alt"></i></a></span>
                                                <span><button class="btn btn-danger btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Excluir" onclick="modalExcluir({{$d->id}}, '{{$d->pergunta}}')"><i class="fas fa-trash-alt"></i></button></span>
                                                @endif
                                            @endif
                                            @if($d->publicado == 1 )
                                                @if(isset($d->respondidos[0]->qtd) && $d->respondidos[0]->qtd >0)
                                                    <span><a href="{{url('/gestao/agenda/enquetes/resultado/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Ver Resultados"><i class="fas fa-chart-pie"></i></a></span>
                                                @else
                                                    <span><button  class="btn btn-success btn-sm fs-13" disabled data-toggle="tooltip" data-placement="top" title="Ver Resultados"><i class="fas fa-chart-pie"></i></button></span>
                                                @endif
                                                <span><a href="{{url('/gestao/agenda/enquetes/respondidos/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Ver Respondidos"><i class="fas fa-clipboard-list"></i></a></span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Exclusão de Enquete</h5>
                </div>
                <div class="modal-body">
                    <form action="">
                        <div class="row">
                            <div class="col-12">
                                Você deseja mesmo excluir esta enquete?<br><br>
    ...
    <div class="modal fade" id="formVisualizar" tabindex="-1" role="dialog" aria-labelledby="formVisualizar" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Visualizar Enquete</h5>
                </div>
                <div class="modal-body">
                    <form action="">
                        <div class="row">
...
<script>
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/enquetes/excluir")}}/'+idExcluir;
    }
    ...
    function publicar()
    {
        window.location.href = '{{url("/gestao/agenda/enquetes/publicar")}}/'+idPublicar;
    }

    function modalVisualizar(id)
    {
        $.ajax({
            url: '{{url('gestao/agenda/enquetes/exibir')}}/'+id,
            type: 'post',
            dataType: 'json',
            data: {
                _token: '{{csrf_token()}}'
