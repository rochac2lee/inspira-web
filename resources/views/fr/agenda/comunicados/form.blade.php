        ...
        <div class="container">
            ...
            <div class="row border-bottom border-top pt-4">
                <div class="col-md-4 pb-2">
                    <h3>
                        <a href="{{ url('/gestao/agenda/comunicados')}}" class="btn btn-secondary">
            ...
            <form id="formCadastroComunicado" action="@if ( strpos(Request::path(),'editar')===false ) {{url('/gestao/agenda/comunicados/add')}} @else {{url('/gestao/agenda/comunicados/editar')}} @endif " method="post" enctype="multipart/form-data">
                ...
                <div class="col-md-6 pl-5">
                    ...
                    <div class="form-group">
                        ...
                        @else
                            <input class="form-control" type="text" disabled  value="Você deverá editar as imagens na opção Gerenciar Imagens a tela anterior.">
                        @endif
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group ">
                        <div class="custom-control custom-switch">
                            ...
                            <small class="form-text w-100 text-muted">
                                <span class="text-danger">Atenção:</span> ação irreversível! Após publicado <b>não será possível desfazer a publicação</b> do Comunicado.
                            </small>
                        </div>
                    </div>
...
<script>
    $(document).ready(function(){
        ...
        @if( ( (isset($dados->turmas) && $dados->turmas) || old('escola') ))
            if($('#visualizacao').val() != 1) {
                $.ajax({
                    url: '{{url('gestao/agenda/comunicados/getEscolasTurmasSelecionados')}}?' + $('#formDadosEditar').serialize(),
                    type: 'post',
                    dataType: 'json',
                    data: {
                        _token: '{{csrf_token()}}'
    ...
    function buscarEscolas(page){
        $.ajax({
            url: '{{url('gestao/agenda/comunicados/getEscolasTurmas')}}?'+$('#formCadastroComunicado input[type=hidden]').serialize(),
            type: 'post',
            dataType: 'json',
            data: {
                nome: $('#buscaNome').val(),
