<div class="row">
    ...
    @if(count($dados->imagens) == 1)
        <div class="col-12 text-center">
            <img class="img-fluid" width="300px" src="{{ config('app.cdn').'/storage/agenda/comunicados/'.$dados->user_id.'/'.$dados->id.'/'.$dados->imagens[0]->caminho}}">
        </div>
            ...
            <section id="banner" class="section">
                <div class="slide-banner">
                    @foreach($dados->imagens as $img)
                        <img class="img-fluid m-2 border" width="200px" src="{{ config('app.cdn').'/storage/agenda/comunicados/'.$dados->user_id.'/'.$dados->id.'/'.$img->caminho}}">
                    @endforeach
                </div>
            </section>
        </div>
