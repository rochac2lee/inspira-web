...
@section('content')
            ...
            <div class="row  pb-4">
                ...
                @if(auth()->user()->permissao != 'P')
                    <div class="col-md-8 text-right">
                        <a href="{{url('gestao/agenda/comunicados/novo')}}" class="btn btn-success" >
                                ...
                                @foreach($dados as $d)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <br><span class="badge badge-light" style="font-weight: normal"><i class="fas fa-user"></i> {{$d->usuario->nome}}</span>
                                        </td>
                                        <td>
                                            ...
                                            @if(auth()->user()->permissao == 'I' || auth()->user()->permissao == 'C')
                                                ...
                                                @if(count($d->escolas)>0)
                                                    <small><i>
                                                            Enviado para:
                                                            @foreach($d->escolas as $e)
                                                                {{$e->titulo}} /
                                                            @endforeach
                                                    </i></small>
                                                @else
                                                    <small><i>Enviado para:  Todas as escolas da instituição</i></small>
                                                @endif
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-right text-nowrap">
                                            ...
                                            @if($d->publicado == 0 && ( auth()->user()->permissao == 'I' ||  (auth()->user()->permissao == 'P' && $d->user_id == auth()->user()->id) || (auth()->user()->permissao == 'C' && ($d->user_id == auth()->user()->id || $d->permissao_usuario == 'P') ) )  )
                                                ...
                                                <span><a href="{{url('/gestao/agenda/comunicados/imagens/'.$d->id)}}"  class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Gerenciar imagens"><i class="fas fa-images"></i> </a></span>
                                                <span><a href="{{url('/gestao/agenda/comunicados/editar/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pencil-alt"></i></a></span>
    ...
    @include('fr.agenda.comunicados.modalPublicar')
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Exclusão de Comunicado</h5>
                </div>
                <div class="modal-body">
                    <form action="">
                        <div class="row">
                            <div class="col-12">
                                Você deseja mesmo excluir esse comunicado?<br><br>
    ...
    <div class="modal fade" id="formVisualizar" tabindex="-1" role="dialog" aria-labelledby="formVisualizar" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Visualizar Comunicado</h5>
                </div>
                <div class="modal-body">
                    <form action="">
                        <div class="row">
...
<script>
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/comunicados/excluir")}}/'+idExcluir;
    }
    ...
    function publicar()
    {
        window.location.href = '{{url("/gestao/agenda/comunicados/publicar")}}/'+idPublicar;
    }

    function modalVisualizar(id)
    {
        $.ajax({
            url: '{{url('gestao/agenda/comunicados/exibir')}}/'+id,
            type: 'post',
            dataType: 'json',
            data: {
                _token: '{{csrf_token()}}'
            ...
            error: function () {
                swal("", "Não foi possível carregar o comunicado.", "error");
            }
        });
