            ...
            <div class="row pb-4">
                ...
                @if(auth()->user()->permissao == 'P' || auth()->user()->permissao == 'C')
                    <div class="col-md-8 text-right">
                        <a href="{{url('gestao/agenda/tarefas/nova')}}" class="btn btn-success" >
                ...
                @elseif(auth()->user()->permissao == 'I')
                    <div class="col-md-8 text-right text-warning">
                        <br>O cadastro de Tarefas e Atividades é realizado por usuários com perfil "Gestor/Coordenador" ou "Docente".
                    </div>
                @endif
            </div>
            <div class="row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if(auth()->user()->permissao == 'I')
                                                ...
                                                <span class="badge badge-light" style="font-weight: normal"><i class="fas fa-user"></i> {{$d->professor->nome}}</span>
                                                <br>
                                            @endif
                                            <span class="badge badge-light" style="font-weight: normal"><i class="fas fa-book"></i> {{$d->disciplina->titulo}}</span>
                                            <span class="badge badge-light" style="font-weight: normal"><i class="far fa-calendar-alt"></i> {{$d->data_entrega->format('d/m/Y')}}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-right text-nowrap">
                                            ...
                                            @if($d->publicado != 1 && ( auth()->user()->permissao == 'C' || (auth()->user()->permissao == 'P' && $d->professor_id == auth()->user()->id )))
                                                ...
                                                <span><a href="{{url('/gestao/agenda/tarefas/editar/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pencil-alt"></i></a></span>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Exclusão de Tarefas e Atividades</h5>
                </div>
                <div class="modal-body">
                    <form action="">
                        <div class="row">
                            <div class="col-12">
                                Você deseja mesmo excluir essa tarefa?<br><br>
...
<script>
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/tarefas/excluir")}}/'+idExcluir;
    }
    ...
    function publicar()
    {
        window.location.href = '{{url("/gestao/agenda/tarefas/publicar")}}/'+idPublicar;
    }

    function modalVisualizar(id)
    {
        $.ajax({
            url: '{{url('gestao/agenda/tarefas/exibir')}}/'+id,
            type: 'post',
            dataType: 'json',
            data: {
                _token: '{{csrf_token()}}'
            ...
            error: function () {
                swal("", "Não foi possível carregar a tarefa.", "error");
            }
        });
