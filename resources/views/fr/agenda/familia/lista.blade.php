...
@section('content')
    <section class="section section-interna" style="padding-top: 50px">
        <div class="container">
            ...
            @endif
            <div class="row mb-3 @if(auth()->user()->permissao != 'Z') border-top @endif pt-4">
                <div class="col-md-12">
                    <div class="filter">
            ...
            <div class="row  pb-3">
                <div class="col-md-8">
                    <h3>
                        Espaço da Família <br>
                        @if(auth()->user()->permissao != 'Z')
                            <small style="font-size: 14px">Clique no botão Visualizar para acessar as publicações realizadas pela Editora Opet. </small>
                        @endif
                    </h3>
                </div>
                @if(auth()->user()->permissao == 'Z')
                    <div class="col-md-4 text-right">
                        <a href="{{url('gestao/agenda/familia/novo')}}" class="btn btn-success" >
                            ...
                            <tbody>
                                @foreach($dados as $d)
                                    <tr>
                                        ...
                                        <td>{{substr($d->descricao,0,150)}}@if(strlen($d->descricao)>150)... @endif</td>
                                        <td>
                                            @if($d->publicado == 1)
                                        ...
                                        <td class="text-right text-nowrap">
                                            ...
                                            @if($d->publicado == 0 && auth()->user()->permissao == 'Z')
                                                ...
                                                <span><a href="{{url('/gestao/agenda/familia/imagens/'.$d->id)}}"  class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Gerenciar imagens"><i class="fas fa-images"></i> </a></span>
                                                <span><a href="{{url('/gestao/agenda/familia/editar/'.$d->id)}}" class="btn btn-success btn-sm fs-13" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pencil-alt"></i></a></span>
...
<script>
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/familia/excluir")}}/'+idExcluir;
    }
    ...
    function publicar()
    {
        window.location.href = '{{url("/gestao/agenda/familia/publicar")}}/'+idPublicar;
    }

    function modalVisualizar(id)
    {
        $.ajax({
            url: '{{url('gestao/agenda/familia/exibir')}}/'+id,
            type: 'post',
            dataType: 'json',
            data: {
                _token: '{{csrf_token()}}'
