...
@section('content')
        ...
        <div class="container">
            ...
            <div class="row mt-4 mb-4">
                <div class="col-12">
                    ...
                    <div class="row mb-4" id="listaImg">
                        ...
                        <div class="col-3 text-center" id="{{$img->id}}">
                            <ul class="list-style">
                                <li >
                                    <img class="img-fluid" width="200px" src="{{ config('app.cdn').'/storage/agenda/familia/'.$dados->user_id.'/'.$dados->id.'/'.$img->caminho}}">
                                    <br>
                ...
                <div class="col-12 text-center">
                    <a href="{{url('/gestao/agenda/familia/')}}" class="btn btn-default" style="float: none">Feito</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <!-- FIM EXCLUIR -->
<script>
    var urlImg = '{{ config('app.cdn').'/storage/agenda/familia/'.$dados->user_id.'/'.$dados->id}}/';

    Dropzone.autoDiscover = false;
    $(document).ready(function(){
        $("#envioImagensDZ").dropzone({
            url: "{{ url('gestao/agenda/familia/imagens/' .$dados->id. '/upload') }}",
            paramName: "file",
            acceptedFiles: 'image/*',
            params:{
                '_token' : '{{csrf_token()}}',
        ...
        $("#listaImg").sortable({
            update: function() {
                ...
                $.ajax({
                    url: '{{ url('gestao/agenda/familia/imagens/' .$dados->id. '/ordem') }}',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        ordem: sort,
    ...
    function excluir()
    {
        window.location.href = '{{url("/gestao/agenda/familia/imagens/".$dados->id."/excluir/")}}/'+idExcluir;
    }
