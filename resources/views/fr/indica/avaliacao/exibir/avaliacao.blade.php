    ...
    <section class="section section-interna" style="padding-top: 30px">
        <div class="container">
            <div class="row title-page">
                <div class="col-8">
                    <h2 >
                        INdica Avaliação - {{$avaliacao->titulo}}
                        <br><span class="badge badge-secondary" style="font-size: 14px">{{$avaliacao->disciplina->titulo}}</span> <span class="badge badge-secondary" style="font-size: 14px">{{$avaliacao->usuario->nome}}</span>
                    </h2>
                </div>
                <div class="col-4">
                    <h2 class="text-right">
...
<section class="section">
    ...
    <div class="container">
        ...
        <h2 class="text-left mt-3" >
            @php $i=1; @endphp
            <form id="formFinalizar" method="post" action="{{url('indica/avaliacao/finalizar')}}">
                @csrf
                    ...
                    <div class="avaliacao-footer mt-4" style="padding-top: 20px; padding-bottom: 20px" >
                        <div class="row">
                            <div class="col-4 text-left" >
                                <button @if($i==1) disabled @endif class="btn btn-outline-secondary btnAnterior" onclick="anterior({{$i}})">Anterior</button>
                            </div>
                            <div class="col-4 text-center">
                                    <button  class="btn btn-warning btnFinalizar" onclick="modalFinalizar()"  style="display: none">Finalizar</button>
                            </div>
                            <div class="col-4 text-right">
                                <button id="btnProximo" class="btn btn-outline-secondary btnProximo" onclick="proximo({{$i}})">Próximo</button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="">
                    <div class="row">
                        <div class="col-12">
                            <P align="justify">Você está prestes a finalizar a avaliação:</P>
                            <p><b class="text-danger">
                                <ul>
                                    <P align="justify"><li> Verifique se todas as questões foram respondidas.</li></P>
                                    <P align="justify"><li> Revise as respostas.</li></P>
                                    <P align="justify"><li> Ao clicar em "Sim, finalizar", sua prova será encerrada. TEM CERTEZA DE QUE DESEJA FINALIZAR?</li></P>
                                </ul>
                            </b> </p>
...
<script>
    ...
    $(document).ready(function(){
        ...
        var obj = new Object();
        obj.indica_questao_id = questaoAtiva;
    ...
    function cronometro() {
        ...
        intervalo = window.setInterval(function() {
            ...
            if(s+(m*60)+(h*3600)>=tempoMaximoAvaliacao*60){
                finalizar();
            }
            s++;
        },1000);
    ...
    function enviaLogGeral(obj,token){
        $.ajax({
            url: '{{url('indica/avaliacao/logGeral')}}',
            type: 'post',
            dataType: 'json',
            data: {
                obj: obj,
    ...
    function enviaLogAtividade(obj,token){
        $.ajax({
            url: '{{url('indica/avaliacao/logAtividade')}}',
            type: 'post',
            dataType: 'json',
            data: {
                obj: obj,
    ...
    $(window).on('beforeunload', function(){
        if(finalizado == 0) {
            ...
            $.ajax({
                url: '{{url('indica/avaliacao/logFechouJanela')}}',
                type: 'post',
                dataType: 'json',
                data: {
                    avaliacao_id: {{$avaliacao->id}},
