...
class AlunoService {
	...
	public function getLista($idEscola, $request = null)
	{
        ...
        /* ANTIGO
		$aluno = User::where('escola_id',$idEscola)
					->where('permissao','A')
					->selectRaw('users.*');

		$alunoPermissao = User::join('user_permissao','user_permissao.user_id','users.id')
					->where('user_permissao.escola_id',$idEscola)
					->where('user_permissao.permissao','A')
					->selectRaw('users.*');

		/// filtro da pesquisa
		if(isset($request) && $request->input('pesquisa')!='')
		{
			$pesquisa = $request->input('pesquisa');
			$aluno->where(function($q) use ($pesquisa) {
				$q->orWhere('nome_completo','like','%'.$pesquisa.'%')
					->orWhere('email','like','%'.$pesquisa.'%')
					->orWhere('users.id',$pesquisa);
			});
			$alunoPermissao->where(function($q) use ($pesquisa) {
				$q->orWhere('nome_completo','like','%'.$pesquisa.'%')
					->orWhere('email','like','%'.$pesquisa.'%')
					->orWhere('users.id',$pesquisa);
			});
		...
		return $aluno->union($alunoPermissao)->orderBy('nome_completo')->paginate(20);
        */
        $alunos =  User::leftJoin('user_permissao','users.id','user_permissao.user_id')
            ->where(function($q) use ($idEscola){
                $q->orWhere(function($qq) use ($idEscola){
                    $qq->where('users.permissao','A')
                        ->where('users.escola_id',$idEscola);
                });
                $q->orWhere(function($qq) use ($idEscola){
            ...
            ->selectRaw('users.*');
        if(isset($request) && $request->input('pesquisa')!=''){
            $buscaNome = $request->input('pesquisa');
	    ...
	    while (!feof($arquivo)) {
	    	...
	    	if($iniciar == 1){ //// se a primeira linha de usuarios for encontrada
	    		if(trim($linha[1]) != ''){ /// se é uma linha preenchida
	        		...
	        		$linhas[] = $this->trataDadosTipoGoogle($linha,$request->input('escola_id'),$count);
	    		}
	    	}
	...
	private function trataDadosTipoGoogle($linha, $idEscola, $count)
	{
